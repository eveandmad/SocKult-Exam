...
simonsen_19 <- read_csv("~/Desktop/SocKult exam/Simonsen_clean.csv")
...
#Loading previously cleaned data
cogsci_18 <- read_csv("~/Desktop/SocKult exam/cogsci_clean.csv")
...
```{r - loading new data}
#Loading previously cleaned data
sc_df <- read_csv("sc_df_clean.csv")
```
...
#Obtaining estimate summaries of the posterior distribution (SM table 5)
summary(Combined_m)
...
#Violin plot of rre- and peri-COVID-19 estimates (uncorrected) with feedback -2.5, 0 and 2.5
...
#Posterior predictive check
(plot_post <- pp_check(meta_model_post_2,nsamples = 100)+ggtitle("Posterior predictive check")+xlab("Change in rating"))
...
#Running simulation of different levels of regression to the mean and true conformity effects
...
#Obtain a table of means of different levels of conformity according to regression to the mean
...
```{r - data wrangling and distraction duration}
#Creating subsets of feedback type
sc_df_low <- subset(sc_df, Feedback %in% c(-3,-2))
sc_df_low$Feedback2 <- -2.5
sc_df_same <- subset(sc_df, Feedback %in% c(-1, 0, 1))
sc_df_same$Feedback2 <- 0
sc_df_high <- subset(sc_df, Feedback %in% c(3,2))
...
#Spaghetti plot of participant level effects (SM figure 11)
...
#Stacked participant level hypothesis plot (SM figure 11)
